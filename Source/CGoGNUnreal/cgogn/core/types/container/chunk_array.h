...
#include <CGoGNUnreal/cgogn/core/types/container/attribute_container.h>
...
public:
	...
	inline std::string name_of_inside_type() override
	{
		return name_of_type<T>::name();
	}

	inline static std::string sname_of_inside_type()
	{
		return name_of_type<T>::name();
	}
	#endif
