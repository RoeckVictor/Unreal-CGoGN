...
#include <CGoGNUnreal/cgogn/core/utils/numerics.h>
#include <CGoGNUnreal/cgogn/core/utils/thread.h>
...
std::shared_ptr<TS> pseudo_pointer_cast(const std::shared_ptr<TG>& x)
{
#ifdef WITH_CGOGN_RTTI
	return std::dynamic_pointer_cast<TS>(x)
#else
	auto s1 = x->name_of_inside_type();
	auto s2 = TS::sname_of_inside_type();
	if (s1 == s2)
		return std::static_pointer_cast<TS>(x);
	return nullptr;
#endif
}
...
#ifndef WITH_CGOGN_RTTI
	virtual std::string name_of_inside_type() = 0;
#endif
protected:
...
public:
	...
	//inline virtual std::string name_of_inside_type()
	//{
	//	return "containerGen";
	//}
...
public:
	...
	std::shared_ptr<Attribute<T>> get_attribute(const std::string& name) const
	{
		...
		if (it != attributes_shared_ptr_.end())
			...
			return pseudo_pointer_cast<typename Attribute<T>,AttributeGenT>(*it);
